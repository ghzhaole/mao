...
classVariantDelegate::classVariantDelegate(QObject *parent) : QItemDelegate(parent) {
...
QWidget *classVariantDelegate::createEditor(QWidget *parent,
  ...
  switch (p->value().type()) {
    ...
    case QVariant::Bool:
    case QVariant::Char:
    case QVariant::String:editor = p->createEditor(parent, option);
      if (editor) {
    ...
    default:editor = QItemDelegate::createEditor(parent, option, index);
  }
  parseEditorHints(editor, p->editorHints());
  return editor;
}

void classVariantDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const {
...
void classVariantDelegate::setModelData(QWidget *editor, QAbstractItemModel *model, const QModelIndex &index) const {
  QVariant data = index.model()->data(index, Qt::EditRole);
  switch (data.type()) {
    case QMetaType::Double:
    case QMetaType::Float:
    ...
    case QVariant::Char:
    case QVariant::String: {
      QVariant data = static_cast<classTreeItem *>(index.internalPointer())->editorData(editor);
      if (data.isValid()) {
...
void classVariantDelegate::updateEditorGeometry(QWidget *editor,
                                                const QStyleOptionViewItem &option,
                                                const QModelIndex &index) const {
  return QItemDelegate::updateEditorGeometry(editor, option, index);
}

void classVariantDelegate::parseEditorHints(QWidget *editor, const QString &editorHints) const {
