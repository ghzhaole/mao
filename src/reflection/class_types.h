...
class classTypes {
 public:
  ...
  bool operator<(const classTypes &ctype) const {
    if (type_ != metaTypes::TYPE_OBJECT) {
      return type_ < ctype.type_;
    } else {
      return object_type_name_ < ctype.object_type_name_;
    }
  }
  ...
  bool operator==(const char *stype) const { return operator==(string(stype)); }
  ...
  const string &object_type_name() const { return object_type_name_; }
