...
namespace mao::json {
	...
	classJson::classJson(const classJson& other) : type_(other.type_) {
		...
		case valueType::INT64_TYPE:
			value_.int64_ = other.value_.int64_;
			break;
		case valueType::FLOAT_TYPE:
			value_.float_ = other.value_.float_;
			break;
		case valueType::DOUBLE_TYPE:
			value_.double_ = other.value_.double_;
			break;
		case valueType::STRING_TYPE:
			value_.string_ = new string(*other.value_.string_);
			break;
		case valueType::LIST_TYPE:
			value_.list_ = new list(*other.value_.list_);
			break;
		case valueType::OBJECT_TYPE:
			value_.object_ = new object(*other.value_.object_);
			break;
		default:
			value_.double_ = 0;
			break;
	...
	classJson::classJson(classJson&& other) : type_(other.type_) {
		...
		case valueType::STRING_TYPE:
			...
			break;
		case valueType::LIST_TYPE:
			value_.list_ = other.value_.list_;
	...
	bool classJson::operator==(const classJson& other) {
		...
		case valueType::OBJECT_TYPE: {
			...
			for (auto&& item : *value_.object_) {
				if (0 == other.value_.object_->count(item.first) ||
					!(other.value_.object_->at(item.first) == item.second)) {
					return false;
				}
			}
			return true;
		...
		case valueType::BOOL_TYPE:
			return value_.bool_ == value_.bool_;
		case valueType::INT_TYPE:
			return value_.int_ == value_.int_;
		case valueType::INT64_TYPE:
			return value_.int64_ == value_.int64_;
		case valueType::FLOAT_TYPE:
			return value_.float_ == value_.float_;
		case valueType::DOUBLE_TYPE:
			return value_.double_ == value_.double_;
		default:
			return true;
		}
	}
	...
	ostream& operator<<(ostream& os, const classJson& cjson) {
		...
		case valueType::NULL_TYPE:
			...
			break;
		case valueType::BOOL_TYPE:
			os << (cjson.value_.bool_ ? "true" : "false");
			break;
		case valueType::INT_TYPE:
			os << cjson.value_.int_;
			break;
		case valueType::INT64_TYPE:
			os << cjson.value_.int64_;
			break;
		case valueType::FLOAT_TYPE: {
			std::stringstream ss;
			ss << std::setprecision(7) << cjson.value_.float_;
		...
		case valueType::DOUBLE_TYPE: {
			std::stringstream ss;
			ss << std::setprecision(15) << cjson.value_.double_;
			if (ss.str().find(".") == std::string::npos)
			{
				ss << ".0";
			}
		...
		case valueType::STRING_TYPE:
			if (cjson.value_.string_) {
				os << "\"" << *cjson.value_.string_ << "\"";
			}
			break;
		case valueType::LIST_TYPE:
			if (cjson.value_.list_) {
				os << "[";
				bool first_ = true;
				for (auto iter = cjson.value_.list_->begin();
					iter != cjson.value_.list_->end(); ++iter) {
					if (!first_) {
						os << ", ";
					}
					else {
			...
			break;
		case valueType::OBJECT_TYPE:
			if (cjson.value_.object_) {
				os << "{";
				bool first_ = true;
				for (auto iter = cjson.value_.object_->begin();
					iter != cjson.value_.object_->end(); ++iter) {
					if (!first_) {
						os << ", ";
					}
					else {
	...
	classJsonParser::classJsonParser(const string& str)
		: str_(str), size_(str.size()) {}
